...
txns_query: str = """SELECT * FROM output.ext.ext_dashboard_view_lc_txns where DATE > '2023-02-01' and CATEGORY = 'TRANSACTIONS';"""

df_txn = pd.DataFrame(fetch_data_from_snowflake(txns_query))
logger.info(print(df_txn.columns))
df_txn.rename(columns={"DATE": "Date",
...
logger.info(print(df_txn.columns))
#%%
df_txn__ttab = df_txn.pivot_table(values="Transaction Count", index=["Retailer"],columns=["Brand"],aggfunc="sum", fill_value=0)
...
df_txn__btab1 = df_txn[df_txn['Date']>last_14].pivot_table(values="Transaction Count", index=["Channel"],columns=["Date"],aggfunc="sum", fill_value=0)
...
df_txn__btab2 = df_txn__btab2.pivot_table(values="Transaction Count", index=["Channel"],columns=["Date"],aggfunc="sum", fill_value=0)
...
layout = dbc.Container([
    ...
    dbc.Row([
        dbc.Col(
                [
                html.H2("Total Matched Transactions by Merchant and Channel",
    ...
    dbc.Row([
        dbc.Col(
            [
                ...
                dcc.Graph(id='txn_by_merchant', figure=px.bar(
    ...
    dbc.Row([
        dbc.Col(
                [
                html.H2("Daily Matched Transactions by Channel",
                className='text-center text-secondary, mb-4', style={'text-align': 'center'}),
                html.Br(),
                dash_table.DataTable(
    ...
    dbc.Row([
        dbc.Col(
                [
                html.H2("Monthly Matched Transactions by Channel",
                className='text-center text-secondary, mb-4', style={'text-align': 'center'}),
                html.Br(),
                dash_table.DataTable(
